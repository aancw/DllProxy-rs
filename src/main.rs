...
fn main() {
	...
	if check_file_exist(&dll_loc){
		...
		match PeFile::from_bytes(&file_map) {
			Ok(Wrap::T32(file)) => c = dump_export32(file),
			Ok(Wrap::T64(file)) => c = dump_export64(file),
			Err(err) => abort(&format!("{}", err)),
		}
        for i in &c {
            // TODO replace PRAGMA with export function
        }
        ...
        println!(
        "{}",
        tmp_format.render_template(&dll_template, &json!({"PRAGMA": "foo"})).unwrap()
        );
        */
